document.addEventListener("DOMContentLoaded", () => {
	...
	function turnClick(action) {
		if (typeof boardSpaces[action.target.id] == "number") {
			if (playMode === 2) {
				let playerTurn = nextPlayer === firstPlayer ? firstPlayer : secondPlayer;
	...
	function gameOver(gameWon) {
	  ...
	  for (let i = 0; i < boxes.length; i++) {
		...
		gameWon.player === firstPlayer ? scores.playerOne +=1 : scores.playerTwo +=1

		if (playMode === 2) {
			declareWinner(gameWon.player == firstPlayer ? 
				`${firstPlayer} won` : `${secondPlayer} won`
			);
		} else {
			declareWinner(gameWon.player == firstPlayer ? 
				`${firstPlayer} Win` : `${firstPlayer} lose`
			);
		}
	}
	...
	function declareWinner(msg) {
		...
		document.querySelector("#playAgain").addEventListener("click", continueGame, false);
	...
	function minimax(newBoard, player) {
		...
		for (let i = 0; i < availSpots.length; i++) {
			...
			if (player === compPlayer)
				...
				move.score =  minimax(newBoard, compPlayer).score;
